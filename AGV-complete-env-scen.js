const { EnvironmentDefinition, EnvironmentConfiguration, Entity, EnvComponent, Event, Scene, Scenario, ScenarioExecution, EventsDefinitions, SceneDefinitions, ScenarioDefinitions, Connection } = require('../sysadl-framework/SysADLBase');
...
// EnvComponent: Station
class Station extends EnvComponent {
  constructor(name, opts = {}) {
    ...
    if (this.envPorts) {
      for (const portName of Object.keys(this.envPorts)) {
        this.envPorts[portName].owner = this;
      }
    }
...
// EnvComponent: PartX
class PartX extends EnvComponent {
  constructor(name, opts = {}) {
    // Initialize default properties structure
    const defaultProperties = {};
    defaultProperties.ID = null; // Type: String
...
// EnvComponent: Lane
class Lane extends EnvComponent {
  constructor(name, opts = {}) {
    const mergedProperties = opts.properties || {};
    
    super(name, {
      ...opts,
      envComponentType: 'Lane',
      properties: mergedProperties,
      state: opts.state || {}
    });
...
// EnvComponent: Supervisory
class Supervisory extends EnvComponent {
  constructor(name, opts = {}) {
    const mergedProperties = opts.properties || {};
    
    // Initialize envPorts with setValue/getValue methods
    const envPorts = {};
    envPorts['inNotification'] = {
      name: 'inNotification',
      value: null,
      owner: null,
      setValue: function(val) { this.value = val; },
...
// EnvComponent: Vehicle
class Vehicle extends EnvComponent {
  constructor(name, opts = {}) {
    // Initialize default properties structure
    const defaultProperties = {};
    defaultProperties.location = null; // Type: String
...
class MyEvents extends EventsDefinitions {
  constructor(name = 'MyEvents', opts = {}) {
      ...
      rules: [
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV2toC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV2toC', {});
              }
            }
            results.push(currentRule.tasks.cmdAGV1toA(context));
            // Emit event 'cmdAGV1toA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV1toA', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV1toC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV1toC', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV1loadA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV1loadA', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV1UnloadA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV1UnloadA', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV2loadC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV2loadC', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV2toE' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV2toE', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'cmdAGV2UnloadE' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('cmdAGV2UnloadE', {});
              }
            }
            return results;
          }
      ...
      rules: [
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1NotifTravelA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1NotifTravelA', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1NotifPassB' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1NotifPassB', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1NotifArriveC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1NotifArriveC', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1NotifArriveA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1NotifArriveA', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1NotifLoad' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1NotifLoad', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1NotifArriveAUnoaded' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1NotifArriveAUnoaded', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1DetectedStationA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1DetectedStationA', {});
              }
            }
            return results;
          }
      ...
      rules: [
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2NotifTravelC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2NotifTravelC', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2NotifArriveC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2NotifArriveC', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2NotifLoad' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2NotifLoad', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2NotifPassD' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2NotifPassD', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2NotifArriveE' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2NotifArriveE', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2NotifArriveAUnoaded' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2NotifArriveAUnoaded', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2DetectedStationD' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2DetectedStationD', {});
              }
            }
            return results;
          }
    ...
    this.StationAEvents = {
      ...
      rules: [
        {
          trigger: 'agv1.sensor == stationA.ID',
          tasks: {
          ...
          execute: (context) => {
            if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('⚡ Executing StationAEvents: agv1.sensor == stationA.ID -> AGV1locationStationA');
            const results = [];
            const currentRule = this.StationAEvents.rules.find(r => r.trigger === 'agv1.sensor == stationA.ID');
            results.push(currentRule.tasks.AGV1locationStationA(context));
            // Emit event 'AGV1locationStationA' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1locationStationA', {});
              }
            }
            return results;
          }
    ...
    this.StationBEvents = {
      ...
      rules: [
        {
          trigger: 'agv1.sensor == stationB.ID',
          tasks: {
          ...
          execute: (context) => {
            if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('⚡ Executing StationBEvents: agv1.sensor == stationB.ID -> AGV1locationStationB');
            const results = [];
            const currentRule = this.StationBEvents.rules.find(r => r.trigger === 'agv1.sensor == stationB.ID');
            results.push(currentRule.tasks.AGV1locationStationB(context));
            // Emit event 'AGV1locationStationB' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1locationStationB', {});
              }
            }
            return results;
          }
    ...
    this.StationCEvents = {
      ...
      rules: [
        {
          trigger: 'agv1.sensor == stationC.ID',
          tasks: {
          ...
          execute: (context) => {
            if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('⚡ Executing StationCEvents: agv1.sensor == stationC.ID -> AGV1locationStationC');
            const results = [];
            const currentRule = this.StationCEvents.rules.find(r => r.trigger === 'agv1.sensor == stationC.ID');
            results.push(currentRule.tasks.AGV1locationStationC(context));
            // Emit event 'AGV1locationStationC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1locationStationC', {});
              }
            }
            return results;
          }
        ...
        {
          trigger: 'agv2.sensor == stationC.ID',
          tasks: {
          ...
          execute: (context) => {
            if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('⚡ Executing StationCEvents: agv2.sensor == stationC.ID -> AGV2locationStationC');
            const results = [];
            const currentRule = this.StationCEvents.rules.find(r => r.trigger === 'agv2.sensor == stationC.ID');
            results.push(currentRule.tasks.AGV2locationStationC(context));
            // Emit event 'AGV2locationStationC' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2locationStationC', {});
              }
            }
            return results;
          }
    ...
    this.StationDEvents = {
      ...
      rules: [
        {
          trigger: 'agv2.sensor == stationD.ID',
          tasks: {
          ...
          execute: (context) => {
            if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('⚡ Executing StationDEvents: agv2.sensor == stationD.ID -> AGV2locationStationD');
            const results = [];
            const currentRule = this.StationDEvents.rules.find(r => r.trigger === 'agv2.sensor == stationD.ID');
            results.push(currentRule.tasks.AGV2locationStationD(context));
            // Emit event 'AGV2locationStationD' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2locationStationD', {});
              }
            }
            return results;
          }
        },
        {
          ...
          tasks: {
            UpdateAGV2SensorToD: (context) => {
              if (context.sysadlBase && context.sysadlBase.helpers) {
                context.sysadlBase.helpers.performAssignment(context, 'agv2.sensor', 'stationD.ID');
              } else {
                agv2.sensor = 'stationD.ID';
              }
              return true;
          ...
          execute: (context) => {
            ...
            // Emit event 'UpdateAGV2SensorToD' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('UpdateAGV2SensorToD', {});
              }
            }
            return results;
          }
    ...
    this.StationEEvents = {
      ...
      rules: [
        {
          trigger: 'agv2.sensor == stationE.ID',
          tasks: {
          ...
          execute: (context) => {
            if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('⚡ Executing StationEEvents: agv2.sensor == stationE.ID -> agv2locationStationE');
            const results = [];
            const currentRule = this.StationEEvents.rules.find(r => r.trigger === 'agv2.sensor == stationE.ID');
            results.push(currentRule.tasks.agv2locationStationE(context));
            // Emit event 'agv2locationStationE' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('agv2locationStationE', {});
              }
            }
            return results;
          }
      ...
      rules: [
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1attachPartX' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1attachPartX', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV1detachPartX' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV1detachPartX', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2attachPartX' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2attachPartX', {});
              }
            }
            return results;
          }
        },
        {
          ...
          execute: (context) => {
            ...
            // Emit event 'AGV2detachPartX' explicitly
            if (context.sysadlBase && context.sysadlBase.eventSystemManager) {
              const globalEmitter = context.sysadlBase.eventSystemManager.getGlobalEmitter();
              if (globalEmitter) {
                globalEmitter.emit('AGV2detachPartX', {});
              }
            }
            return results;
          }
...
class MyScenariosExecution extends ScenarioExecution {
  ...
  async executeAsync(context) {
    ...
    // inject SetAGV2SensorStationD when agv1.location == stationA.ID;
    if (context.eventScheduler) {
      context.eventScheduler.scheduleOnCondition('SetAGV2SensorStationD', () => context.model?.environments?.['MyFactoryConfiguration']?.agv1?.location == context.model?.environments?.['MyFactoryConfiguration']?.stationA?.ID);
    }
    // inject AGV1atStationA after cmdAGV1toA;
    // inject AGV1atStationA after cmdAGV1toA;
    if (context.eventScheduler) {
      context.eventScheduler.scheduleOnCondition('AGV1atStationA', () => {
        try {
          const agv1 = context.model?.environments?.['MyFactoryConfiguration']?.agv1;
          const notification = agv1?.properties?.outNotification?.notification;
          return notification === "traveling";
